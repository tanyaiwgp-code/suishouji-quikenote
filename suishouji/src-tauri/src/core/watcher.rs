...
/// 递归监听 root；外部修改后（去抖）调用 on_change。阻塞运行直到 channel 断开。
pub fn spawn(root: PathBuf, mut on_change: impl FnMut() + Send + 'static) {
    std::thread::spawn(move || {
        ...
        loop {
            match rx.recv_timeout(Duration::from_millis(DEBOUNCE_MS)) {
                Ok(_) => pending = true, // 又有新事件，重置静默窗口
...
mod tests {
    ...
    fn spawn_with_missing_root_is_graceful() {
        ...
        spawn(root.clone(), move || flag.store(true, Ordering::SeqCst));
